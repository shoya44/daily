...
Dailyの中心画面。「今日」「明日」の2枚のカードのみで構成し、スクロールなしで概ね見渡せることを目指す。各カードは同じ構成（天気→勤務→ゴミ→予定）で統一し、視線移動を減らす。

カードごとの表示項目:

1. **天気** — 天気アイコン、最高気温、天気説明、傘の判断、服装の判断
2. **勤務** — その日の勤務形態（タップで変更）。今日カードのみ、今週の残り出社日数を併記
3. **ゴミ** — その日出すゴミ
4. **予定** — その日の予定・メモ（タップで編集）
...
- タップで編集（本文変更・削除が可能）
- 編集・追加結果は短いToastで通知
- **予定の新規追加はFAB（右下＋）からのみ行う。** カード内に追加用の入力欄・ボタンは置かない
- FABタップ→日付選択（今日〜来週の日曜まで）→入力→Enterまたは追加ボタン、の順で登録する
...
来週以降の予定を含め、複数日先の予定をまとめて見たい場合はカレンダー画面を使う。
...
- [ ] Today画面が「今日」「明日」の2カードのみで、スクロールなしで概ね見渡せる
- [ ] 傘が必要な日は「傘を持っていく」と表示される（今日・明日それぞれ）
- [ ] 服装アドバイスが今日・明日それぞれに表示される
- [ ] 今週あと何日出社かが今日カードのみに表示される
- [ ] ゴミの日を見落としにくい（今日・明日それぞれ）
- [ ] 今日・明日それぞれの予定が確認できる
- [ ] 今日・明日どちらの勤務バッジをタップしても勤務形態を変更できる
- [ ] 右下のFABが他要素と重ならず、常に操作できる
- [ ] FAB→日付選択（今日〜来週の日曜）→入力→Enterで予定を追加できる
- [ ] 予定の新規追加はFAB以外から行えない（カード内に追加ボタンがない）
