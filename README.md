...
独立した詳細仕様書は [`SPEC.md`](./SPEC.md) にもまとめている。

READMEでは、セットアップ、運用、設計方針、画面仕様、判定ロジック、エラー処理、PWA、通知、受け入れテストまでを一つにまとめ、READMEだけでDailyの全体像を把握できる構成としている。
